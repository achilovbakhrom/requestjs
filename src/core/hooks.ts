...
type HookReturnType<T> = [callApi: () => void, isLoading: boolean, response: T | undefined];

export const useGet = <PARAMS, RESPONSE_BODY>(url: string, params: PARAMS, headers: HeaderType = null): HookReturnType<RESPONSE_BODY> => {
    ...
    const callApi = useCallback<(params: PARAMS) => void>((params) => {
        setIsLoading(true)
        requestClient
...
export const usePostJSON = <BODY, RESPONSE_BODY>(url: string, body: BODY, headers: HeaderType = null): HookReturnType<RESPONSE_BODY> => {
    ...
    const callApi = useCallback<(body: BODY) => void>((body) => {
        setIsLoading(true)
        requestClient
